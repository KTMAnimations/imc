...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # Log market_trades for bot behavior analysis
        if state.timestamp <= 20000:
            for product in ["TOMATOES", "EMERALDS"]:
                mts = state.market_trades.get(product, [])
                for t in mts:
                    print(f"MT|{state.timestamp}|{product}|{t.buyer}|{t.seller}|{t.price}|{t.quantity}")

            # Log order book snapshot for context
            for product in ["TOMATOES", "EMERALDS"]:
                od = state.order_depths.get(product)
                if od:
                    bb = max(od.buy_orders.keys()) if od.buy_orders else 0
                    ba = min(od.sell_orders.keys()) if od.sell_orders else 0
                    print(f"OB|{state.timestamp}|{product}|{bb}|{ba}")

            # Log own_trades to see what filled
            for product in ["TOMATOES", "EMERALDS"]:
                ots = state.own_trades.get(product, [])
                for t in ots:
                    print(f"OT|{state.timestamp}|{product}|{t.buyer}|{t.seller}|{t.price}|{t.quantity}")
