"""
IMC Prosperity 2 - jmerle (9th Place Global)
Round 3: Gift Basket Threshold Trading
Source: https://github.com/jmerle/imc-prosperity-2/blob/master/src/submissions/round3.py

Strategy:
- Computes diff = GIFT_BASKET_mid - (4*CHOCOLATE_mid + 6*STRAWBERRIES_mid + 1*ROSES_mid)
- Each product (CHOCOLATE, STRAWBERRIES, ROSES, GIFT_BASKET) has its own long/short thresholds
- When diff < long_threshold: go 100% long in that product
- When diff > short_threshold: go 100% short in that product
- Uses the "worst" prices in the book (max ask / min bid) for aggressive fills
- No rolling z-score needed -- pure static threshold approach tuned via grid search
"""
    ...
    def compress_observations(self, observations: Observation) -> list[Any]:
        ...
        for product, observation in observations.conversionObservations.items():
            conversion_observations[product] = [
                ...
                observation.sunlight, observation.humidity,
            ]
...
class GiftBasketStrategy(Strategy):
    ...
    - Computes diff = GIFT_BASKET_mid - (4*CHOCOLATE + 6*STRAWBERRIES + 1*ROSES)
    - Each product has different long/short thresholds tuned via grid search
    - Goes 100% long or 100% short based on whether diff crosses thresholds
    """
    def act(self, state: TradingState) -> None:
