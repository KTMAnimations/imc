"""
Parametric template for v4 optimization. Params are at the top so a
sed-based substitution can swap values for sweeps. T_BBMID_WEIGHT=0 makes
this identical to baseline tradestrategy.py.
"""
...
EMERALDS_MAKE_EDGE = 7  # PARAM
EMERALDS_SOFT_LIMIT = 35  # PARAM
EMERALDS_HARD_LIMIT = 60  # PARAM

TOMATOES_LIMIT = 80
TOMATOES_EMA_ALPHA = 0.50  # PARAM
TOMATOES_INV_SKEW = 0.05  # PARAM
TOMATOES_SOFT_LIMIT = 40  # PARAM
TOMATOES_HARD_LIMIT = 60  # PARAM
TOMATOES_PASSIVE_CAP = 20  # PARAM
TOMATOES_BBMID_WEIGHT = 0.00  # PARAM
...
class Trader:
    ...
    def tomatoes_fair(self, od, pos, trader_data):
        ...
        blended = (1 - TOMATOES_BBMID_WEIGHT) * wall_mid + TOMATOES_BBMID_WEIGHT * bb_mid
        alpha = TOMATOES_EMA_ALPHA
        prev = trader_data.get("tomatoes_ema")
        if prev is not None:
            ema = prev * (1 - alpha) + blended * alpha
        else:
            ema = blended
