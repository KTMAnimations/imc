"""
v5c: Trend-filtered take. Maintains a fast and slow EMA of wall_mid.
trend = fast_ema - slow_ema. If trend > T_TREND_THR, don't take sells
(market rising — don't fight the trend). If trend < -T_TREND_THR, don't
take buys.
"""
...
TOMATOES_EMA_FAST = 0.7
TOMATOES_EMA_SLOW = 0.2
TOMATOES_TREND_THR = 0.5
...
class Trader:
    ...
    def emeralds_take(self, od, pos, bv, sv):
        ...
        for ask in sorted(od.sell_orders.keys()):
            ...
            vol = -od.sell_orders[ask]; qty = min(vol, EMERALDS_LIMIT - pos - bv)
            if qty > 0:
                orders.append(Order("EMERALDS", ask, qty)); bv += qty
        ...
        for bid in sorted(od.buy_orders.keys(), reverse=True):
            ...
            vol = od.buy_orders[bid]; qty = min(vol, EMERALDS_LIMIT + pos - sv)
            if qty > 0:
                orders.append(Order("EMERALDS", bid, -qty)); sv += qty
    ...
    def emeralds_clear(self, od, pos, bv, sv):
        ...
        if net > 0:
            ...
            if qty > 0: orders.append(Order("EMERALDS", price, -qty)); sv += qty
        elif net < 0:
            price = EMERALDS_FAIR - EMERALDS_CLEAR_WIDTH
            avail = sum(-v for p,v in od.sell_orders.items() if p <= price)
            qty = min(avail, -net, EMERALDS_LIMIT - pos - bv)
            if qty > 0: orders.append(Order("EMERALDS", price, qty)); bv += qty
        return orders, bv, sv
    ...
    def tomatoes_fair(self, od, pos, trader_data):
        if not od.sell_orders or not od.buy_orders: return None, trader_data, 0.0
        ...
        ema = prev * (1 - TOMATOES_EMA_ALPHA) + wall_mid * TOMATOES_EMA_ALPHA if prev is not None else wall_mid
        trader_data["tomatoes_ema"] = ema
        # Fast and slow trend EMAs
        fast = trader_data.get("tomatoes_fast")
        slow = trader_data.get("tomatoes_slow")
        fast = (1 - TOMATOES_EMA_FAST) * fast + TOMATOES_EMA_FAST * wall_mid if fast is not None else wall_mid
        slow = (1 - TOMATOES_EMA_SLOW) * slow + TOMATOES_EMA_SLOW * wall_mid if slow is not None else wall_mid
        trader_data["tomatoes_fast"] = fast
        trader_data["tomatoes_slow"] = slow
        trend = fast - slow
        fair = ema - TOMATOES_INV_SKEW * pos
        return fair, trader_data, trend

    def tomatoes_take(self, od, fair, pos, bv, sv, trend):
        orders = []
        # If market trending UP (trend > thr), skip sells (don't fade rally)
        # If trending DOWN (trend < -thr), skip buys (don't catch falling knife)
        skip_sells = trend > TOMATOES_TREND_THR
        skip_buys = trend < -TOMATOES_TREND_THR
        if not skip_buys:
            for ask in sorted(od.sell_orders.keys()):
                if ask >= fair: break
                ...
                if qty > 0:
                    orders.append(Order("TOMATOES", ask, qty)); bv += qty
                    od.sell_orders[ask] += qty
                    if od.sell_orders[ask] == 0: del od.sell_orders[ask]
        if not skip_sells:
            for bid in sorted(od.buy_orders.keys(), reverse=True):
                ...
                if qty > 0:
                    orders.append(Order("TOMATOES", bid, -qty)); sv += qty
                    od.buy_orders[bid] -= qty
                    if od.buy_orders[bid] == 0: del od.buy_orders[bid]
        return orders, bv, sv
    ...
    def tomatoes_clear(self, od, fair, pos, bv, sv):
        ...
        if net > 0:
            ...
            if qty > 0: orders.append(Order("TOMATOES", price, -qty)); sv += qty
        elif net < 0:
            price = self.tomatoes_clear_price(fair, net)
            avail = sum(-v for p,v in od.sell_orders.items() if p <= price)
            qty = min(avail, -net, TOMATOES_LIMIT - pos - bv)
            if qty > 0: orders.append(Order("TOMATOES", price, qty)); bv += qty
        return orders, bv, sv
    ...
    def tomatoes_make(self, od, fair, pos, bv, sv):
        ...
        half = TOMATOES_LIMIT // 2; base = TOMATOES_PASSIVE_CAP
    ...
    def trade_tomatoes(self, state, trader_data):
        ...
        fair, trader_data, trend = self.tomatoes_fair(od, pos, trader_data)
        if fair is None: return [], trader_data
        bv = sv = 0
        take, bv, sv = self.tomatoes_take(od, fair, pos, bv, sv, trend)
