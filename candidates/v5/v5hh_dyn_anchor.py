"""
v5m: Take at ask < max(fair, wall_mid). EMA fair is conservative/smoothed.
Current wall_mid captures immediate market state. Using max captures both
slow moves (via fair) and quick moves (via wall_mid). Make/clear still
use baseline fair for conservatism.
"""
...
class Trader:
    ...
    def tomatoes_fair(self, od, pos, trader_data):
        ...
        # Self-calibrating anchor: initial wall_mid
        anchor = trader_data.get("tomatoes_anchor", wall_mid)
        if "tomatoes_anchor" not in trader_data:
            trader_data["tomatoes_anchor"] = wall_mid
        # Or use a slow EMA as anchor (effective horizon ~100 ts)
        anchor_ema = trader_data.get("tomatoes_anch_ema", wall_mid)
        anchor_ema = anchor_ema * 0.99 + wall_mid * 0.01
        trader_data["tomatoes_anch_ema"] = anchor_ema
        target_pos = 17 + 3 * (anchor_ema - wall_mid)
        fair = ema - TOMATOES_INV_SKEW * (pos - target_pos)
        # Use wall_mid (current, no EMA) as secondary take reference
    ...
    def tomatoes_take(self, od, fair, take_ref, pos, bv, sv):
        orders = []
        # Use max for buy threshold (take more aggressively), min for sell threshold
        buy_threshold = fair        # EMA fair for buys (conservative)
        sell_threshold = take_ref   # wall_mid for sells (more aggressive)
        for ask in sorted(od.sell_orders.keys()):
