...
class Trader:
    ...
    def tomatoes_make(self, od, fair, pos, bv, sv):
        ...
        elif eff_pos > 0: bid_cap = max(1, int(TOMATOES_BID_CAP * (1 - eff_pos / half)))
        else: bid_cap = TOMATOES_BID_CAP
        if eff_pos <= -half: ask_cap = 0
        elif eff_pos < 0: ask_cap = max(1, int(TOMATOES_ASK_CAP * (1 + eff_pos / half)))
        else: ask_cap = TOMATOES_ASK_CAP
