"""
v5i: Explicit position targeting. target_pos = K * (fair - bb_mid).
When fair > bb_mid (walls above visible market), be long (price will rise to walls).
When fair < bb_mid, be short.
The make/clear/take phases all work toward this target.
"""
...
TOMATOES_TARGET_K = 8.0  # target_pos = K * (fair - mid)
...
class Trader:
    ...
    def tomatoes_fair(self, od, pos, trader_data):
        ...
        # target_pos = K * (fair - bb_mid)
        target_pos = round(TOMATOES_TARGET_K * (ema - bb_mid))
        target_pos = max(-TOMATOES_LIMIT, min(TOMATOES_LIMIT, target_pos))
        return fair, target_pos, trader_data

    def tomatoes_take(self, od, fair, pos, bv, sv, target_pos):
        orders = []
        # Take buys only if pos < target_pos
        # Take sells only if pos > target_pos
        if pos + bv - sv < target_pos:
            for ask in sorted(od.sell_orders.keys()):
                if ask >= fair: break
                if pos + bv - sv >= target_pos: break  # reached target
                vol = -od.sell_orders[ask]
                qty = min(vol, TOMATOES_LIMIT - pos - bv, target_pos - (pos + bv - sv))
                if qty > 0:
        ...
        if pos + bv - sv > target_pos:
            for bid in sorted(od.buy_orders.keys(), reverse=True):
                if bid <= fair: break
                if pos + bv - sv <= target_pos: break
                vol = od.buy_orders[bid]
                qty = min(vol, TOMATOES_LIMIT + pos - sv, (pos + bv - sv) - target_pos)
                if qty > 0:
    ...
    def trade_tomatoes(self, state, trader_data):
        ...
        fair, target_pos, trader_data = self.tomatoes_fair(od, pos, trader_data)
        if fair is None: return [], trader_data
        bv = sv = 0
        take, bv, sv = self.tomatoes_take(od, fair, pos, bv, sv, target_pos)
