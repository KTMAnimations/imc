"""
v5f: max_bid / min_ask use bb_mid (visible mid) instead of wall_mid_EMA fair.
The fair is still wall_mid_EMA for take decisions. Idea: quote prices
should react to visible market, take decisions should use stable wall fair.
"""
...
class Trader:
    ...
    def tomatoes_quote_prices(self, od, fair, eff_pos, quote_fair):
        """quote_fair is used for max_bid/min_ask; fair is for clear price."""
        best_bid = max(od.buy_orders.keys()) if od.buy_orders else None
        best_ask = min(od.sell_orders.keys()) if od.sell_orders else None
        if best_bid is None or best_ask is None: return None, None
        max_bid = math.ceil(quote_fair) - 1
        min_ask = math.floor(quote_fair) + 1
    ...
    def tomatoes_fair(self, od, pos, trader_data):
        ...
        # quote_fair uses the visible bb_mid (responsive to quotes)
        quote_fair = bb_mid - TOMATOES_INV_SKEW * pos
        return fair, quote_fair, trader_data
    ...
    def tomatoes_make(self, od, fair, pos, bv, sv, quote_fair):
        orders = []
        eff_pos = pos + bv - sv
        bid, ask = self.tomatoes_quote_prices(od, fair, eff_pos, quote_fair)
    ...
    def trade_tomatoes(self, state, trader_data):
        ...
        fair, quote_fair, trader_data = self.tomatoes_fair(od, pos, trader_data)
        ...
        make = self.tomatoes_make(od, fair, pos, bv, sv, quote_fair)
