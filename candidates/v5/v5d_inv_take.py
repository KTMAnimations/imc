"""
v5d: Inventory-conditional take. Only take aggressive sells when current
pos is long; only take buys when current pos is short. The idea is that
take should be position-reducing, not position-creating.
"""
...
class Trader:
    ...
    def tomatoes_take(self, od, fair, pos, bv, sv):
        orders = []
        eff_pos = pos + bv - sv
        # Inventory-conditional: take BUYs only when not already long, etc.
        # Allow take BUY if eff_pos <= 0 (we're short or neutral, want to cover/build long)
        # Allow take SELL if eff_pos >= 0 (we're long or neutral, want to unwind/build short)
        if eff_pos <= 0:
            for ask in sorted(od.sell_orders.keys()):
                if ask >= fair: break
