...
class Trader:
    ...
    def tomatoes_take(self, od, fair, take_ref, pos, bv, sv):
        orders = []
        _aw = min(od.sell_orders, key=lambda p: -od.sell_orders[p]) if od.sell_orders else None
        ...
        for bid in sorted(od.buy_orders.keys(), reverse=True):
            ...
            if qty > 0:
                ...
                od.buy_orders[bid] -= qty
                if od.buy_orders[bid] == 0: del od.buy_orders[bid]
        # Phase 2: take asks below ask_wall, cap 5
        if _aw is not None:
            extra = 5
            for ask in sorted(od.sell_orders.keys()):
                if ask >= _aw or extra <= 0: break
                vol = -od.sell_orders[ask]
                qty = min(vol, TOMATOES_LIMIT - pos - bv, extra)
                if qty > 0:
                    orders.append(Order("TOMATOES", ask, qty))
                    bv += qty
                    extra -= qty
