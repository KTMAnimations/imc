...
class Trader:
    ...
    def tomatoes_fair(self, od, pos, trader_data):
        ...
        bb_mid = (max(od.buy_orders.keys()) + min(od.sell_orders.keys())) / 2
        blended = 0.8 * wall_mid + 0.2 * bb_mid
        alpha = TOMATOES_EMA_ALPHA
        prev = trader_data.get("tomatoes_ema")
        ema = prev*(1-alpha) + blended*alpha if prev is not None else blended
        trader_data["tomatoes_ema"] = ema
        fair = ema - TOMATOES_INV_SKEW * (pos - 17)
        # Use wall_mid (current, no EMA) as secondary take reference
        take_ref = wall_mid - TOMATOES_INV_SKEW * (pos - 17)
