...
class Trader:
    ...
    def tomatoes_fair(self, od, pos, trader_data):
        ...
        return fair, take_ref, bid_wall, ask_wall, trader_data

    def tomatoes_take(self, od, fair, take_ref, bid_wall, ask_wall, pos, bv, sv):
        orders = []
        # Use max for buy threshold (take more aggressively), min for sell threshold
        buy_threshold = fair        # EMA fair for buys (conservative)
        sell_threshold = bid_wall    # any bid above bid_wall = spike
        for ask in sorted(od.sell_orders.keys()):
    ...
    def trade_tomatoes(self, state, trader_data):
        ...
        fair, take_ref, bid_wall, ask_wall, trader_data = self.tomatoes_fair(od, pos, trader_data)
        if fair is None: return [], trader_data
        bv = sv = 0
        take, bv, sv = self.tomatoes_take(od, fair, take_ref, bid_wall, ask_wall, pos, bv, sv)
