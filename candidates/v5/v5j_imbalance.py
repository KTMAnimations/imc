"""
v5j: Order book imbalance signal. If visible bid volume >> ask volume,
more buying pressure → shift fair up (buy more aggressively). Vice versa.
"""
...
TOMATOES_IMBALANCE_K = 1.0  # how much book imbalance shifts fair
...
class Trader:
    ...
    def tomatoes_fair(self, od, pos, trader_data):
        ...
        # Imbalance: compare total visible bid vs ask volume
        bid_vol = sum(od.buy_orders.values())
        ask_vol = sum(-v for v in od.sell_orders.values())
        total = bid_vol + ask_vol
        if total > 0:
            imbalance = (bid_vol - ask_vol) / total  # in [-1, +1]
        else:
            imbalance = 0.0
        ...
        fair = ema - TOMATOES_INV_SKEW * pos + TOMATOES_IMBALANCE_K * imbalance
