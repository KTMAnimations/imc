"""
v5h: Momentum-filtered take. Track last K bb_mids; momentum = current - K_ago.
Skip take sells when momentum > thr (rising); skip take buys when momentum < -thr (falling).
"""
...
class Trader:
    ...
    def tomatoes_fair(self, od, pos, trader_data):
        ...
        # Track recent bb_mids
        history = trader_data.get("tomatoes_mid_hist", [])
        history.append(bb_mid)
        if len(history) > TOMATOES_MOM_LAG + 1:
            history = history[-(TOMATOES_MOM_LAG + 1):]
        trader_data["tomatoes_mid_hist"] = history
        if len(history) > TOMATOES_MOM_LAG:
            momentum = history[-1] - history[-TOMATOES_MOM_LAG - 1]
        else:
            momentum = 0.0
        ...
        return fair, trader_data, momentum

    def tomatoes_take(self, od, fair, pos, bv, sv, momentum):
        orders = []
        skip_sells = momentum > TOMATOES_MOM_THR
        skip_buys = momentum < -TOMATOES_MOM_THR
        if not skip_buys:
            for ask in sorted(od.sell_orders.keys()):
                if ask >= fair: break
    ...
    def trade_tomatoes(self, state, trader_data):
        ...
        fair, trader_data, momentum = self.tomatoes_fair(od, pos, trader_data)
        if fair is None: return [], trader_data
        bv = sv = 0
        take, bv, sv = self.tomatoes_take(od, fair, pos, bv, sv, momentum)
