"""
v5b: Momentum-biased TOMATOES fair.
Track wall_mid changes over 2 ts. If increasing, add a small upward bias
to fair (be more willing to buy, less willing to sell). If decreasing,
reverse. This should capture trend-following gains.
"""
...
TOMATOES_MOMENTUM_K = 0.30  # how much the wall_mid delta contributes to fair
...
class Trader:
    ...
    def tomatoes_fair(self, od, pos, trader_data):
        ...
        # Momentum: last wall_mid vs current
        prev_wall = trader_data.get("tomatoes_prev_wall")
        momentum = 0.0
        if prev_wall is not None:
            momentum = wall_mid - prev_wall
        trader_data["tomatoes_prev_wall"] = wall_mid
        trader_data["tomatoes_ema"] = ema
        fair = ema - TOMATOES_INV_SKEW * pos + TOMATOES_MOMENTUM_K * momentum
