"""
v5k: Take spike-side liquidity. When best_ask < ask_wall, there's a
small aggressive sell order below the main wall (a spike). Taking it
captures the spike before it disappears. Similarly for bid spikes.

The take phase now takes:
1. Anything strictly below fair (baseline)
2. Spikes: asks below ask_wall (even if >= fair), and bids above bid_wall
"""
...
TOMATOES_SPIKE_CAP = 20  # max extra qty per side via spike-take
...
class Trader:
    ...
    def tomatoes_fair(self, od, pos, trader_data):
        if not od.sell_orders or not od.buy_orders: return None, trader_data, None, None
        ...
        return fair, trader_data, bid_wall, ask_wall

    def tomatoes_take(self, od, fair, bid_wall, ask_wall, pos, bv, sv):
        orders = []
        # Phase 1: baseline take (ask < fair, bid > fair)
        for ask in sorted(od.sell_orders.keys()):
            if ask >= fair: break
            vol = -od.sell_orders[ask]
        ...
        # Phase 2: spike take. If asks exist strictly below ask_wall (i.e.,
        # there's a small aggressive sell order under the main ask wall),
        # take those asks. Cap at TOMATOES_SPIKE_CAP per side.
        extra_buy_budget = TOMATOES_SPIKE_CAP
        for ask in sorted(od.sell_orders.keys()):
            if ask >= ask_wall or extra_buy_budget <= 0: break
            vol = -od.sell_orders[ask]
            qty = min(vol, TOMATOES_LIMIT - pos - bv, extra_buy_budget)
            if qty > 0:
                orders.append(Order("TOMATOES", ask, qty))
                bv += qty
                extra_buy_budget -= qty
                od.sell_orders[ask] += qty
                if od.sell_orders[ask] == 0: del od.sell_orders[ask]

        extra_sell_budget = TOMATOES_SPIKE_CAP
        for bid in sorted(od.buy_orders.keys(), reverse=True):
            if bid <= bid_wall or extra_sell_budget <= 0: break
            vol = od.buy_orders[bid]
            qty = min(vol, TOMATOES_LIMIT + pos - sv, extra_sell_budget)
            if qty > 0:
                orders.append(Order("TOMATOES", bid, -qty))
                sv += qty
                extra_sell_budget -= qty
                od.buy_orders[bid] -= qty
                if od.buy_orders[bid] == 0: del od.buy_orders[bid]
    ...
    def trade_tomatoes(self, state, trader_data):
        ...
        fair, trader_data, bid_wall, ask_wall = self.tomatoes_fair(od, pos, trader_data)
        if fair is None: return [], trader_data
        bv = sv = 0
        take, bv, sv = self.tomatoes_take(od, fair, bid_wall, ask_wall, pos, bv, sv)
