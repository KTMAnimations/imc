...
class Trader:
    ...
    def tomatoes_fair(self, od, pos, trader_data):
        ...
        # Long-term anchor (0.01 alpha, ~100 ts horizon)
        anchor_prev = trader_data.get("tomatoes_anchor")
        anchor = anchor_prev*0.99 + wall_mid*0.01 if anchor_prev is not None else wall_mid
        trader_data["tomatoes_anchor"] = anchor
        # Mean reversion: bias fair toward anchor
        fair = ema - TOMATOES_INV_SKEW * pos + 0.1 * (anchor - ema)
        # Use wall_mid (current, no EMA) as secondary take reference
        take_ref = wall_mid - TOMATOES_INV_SKEW * pos
