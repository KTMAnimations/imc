"""
v5a: Microprice fair model.

fair_input = (best_bid * ask_vol + best_ask * bid_vol) / (ask_vol + bid_vol)
EMA over that. Microprice is the volume-weighted mid. When bid volume
dominates, microprice tilts toward best_ask (predicting upward); when
ask volume dominates, toward best_bid.

Other params unchanged from baseline.
"""
...
class Trader:
    ...
    def tomatoes_fair(self, od, pos, trader_data):
        ...
        # Microprice: volume-weighted toward the side with LESS volume
        # (because that side will be filled first if pressure persists).
        ...
        ema = prev * (1 - alpha) + microprice * alpha if prev is not None else microprice
