...
class Trader:
    ...
    def tomatoes_fair(self, od, pos, trader_data):
        ...
        # Inventory-adjusted fair for taking/making; raw EMA for clearing
        fair = ema - TOMATOES_INV_SKEW * pos

        return ema, fair, trader_data
    ...
    def trade_tomatoes(self, state, trader_data):
        ...
        ema, fair, trader_data = self.tomatoes_fair(od, pos, trader_data)
        if fair is None:
            return [], trader_data

        bv = sv = 0
        take, bv, sv = self.tomatoes_take(od, ema, pos, bv, sv)
        clear, bv, sv = self.tomatoes_clear(od, ema, pos, bv, sv)
